...
struct RimindSetSundayView5: View {
    ...
    //　フィルター
    @ObservedResults(RimindDrugDB.self,where: {$0.rimindTime == "昼食前" && $0.rimindDay == "金曜日"}) var rimaindGroups1
    @ObservedResults(RimindDrugDB.self,where: {$0.rimindTime == "昼食後" && $0.rimindDay == "金曜日"}) var rimaindGroups2
    @ObservedResults(RimindDrugDB.self,where: {$0.rimindTime == "間食" && $0.rimindDay == "金曜日"}) var rimaindGroups3
                ...
                if rimaindGroups1.count != 0 {
                    ScrollViewReader { reader in List {
                        ...
                        ForEach(rimaindGroups1) { item in
                            ...
                            if item.rimindTime == "昼食前" && item.rimindDay == "金曜日"{
                                Text(item.name)
                                    .font(.largeTitle)
                ...
                if rimaindGroups2.count != 0 {
                    ScrollViewReader { reader in List {
                        ...
                        ForEach(rimaindGroups2) { item in
                            ...
                            if item.rimindTime == "昼食後" && item.rimindDay == "金曜日"{
                                Text(item.name)
                                    .font(.largeTitle)
                                    .listRowBackground(Color(red: item.drugColorRed, green: item.drugColorGreen, blue: item.drugColorBrue))
                                    .onTapGesture {
                ...
                if rimaindGroups3.count != 0 {
                    ScrollViewReader { reader in List {
                        ...
                        ForEach(rimaindGroups3) { item in
                            ...
                            if item.rimindTime == "間食" && item.rimindDay == "金曜日"{
                                Text(item.name)
                                    .font(.largeTitle)
                                    .listRowBackground(Color(red: item.drugColorRed, green: item.drugColorGreen, blue: item.drugColorBrue))
                                    .onTapGesture {
...
struct RimindSetSundayView5_Previews: PreviewProvider {
    static var previews: some View {
        RimindSetSundayView5()
    }
}


extension RimindSetSundayView5 {
